...
function GameCard({ game, standingsMap}: { 
  game: Game; 
...
// });
  ...
  const homeEmojis = teamEmojis[game.homeTeam.abbrev] || "🏒";
    ...
    // Obtenir les infos de classement pour chaque équipe
  const awayStanding = standingsMap?.get(game.awayTeam.abbrev);
  const homeStanding = standingsMap?.get(game.homeTeam.abbrev);
  
  const awayClinching = getClinchingStatus(awayStanding);
  const homeClinching = getClinchingStatus(homeStanding);
  const awayPosition = getPositionLabel(awayStanding);
  const homePosition = getPositionLabel(homeStanding);
  
  // Détecter si c'est un match de playoffs (gameType === 3)
  const isPlayoff = game.gameType === 3;
  ...
  return (
    <div className="group relative bg-gray-800/50 backdrop-blur-sm rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]">
      ...
      {/* Affichage des positions en saison régulière */}
{!isPlayoff && (awayPosition || homePosition) && !isLive && (
  <div className="flex justify-center gap-4 mt-2 mb-1 text-xs">
    {awayPosition && (
      <div className={`flex items-center gap-1 ${awayPosition.isWildcard ? 'text-orange-400' : 'text-blue-400'}`}>
        <span>#{awayPosition.text}</span>
        <span className="text-gray-600 cursor-help" title={awayPosition.tooltip}>ⓘ</span>
      </div>
    )}
    <span className="text-gray-600">•</span>
    {homePosition && (
      <div className={`flex items-center gap-1 ${homePosition.isWildcard ? 'text-orange-400' : 'text-blue-400'}`}>
        <span>#{homePosition.text}</span>
        <span className="text-gray-600 cursor-help" title={homePosition.tooltip}>ⓘ</span>
      </div>
    )}
  </div>
)}
...
{(awayClinching || homeClinching) && !isPlayoff && !isLive && (
  <div className="mt-2 text-center">
    <div className="flex justify-center gap-3 text-xs">
      {awayClinching && (
        <span className={`${awayClinching.color} flex items-center gap-1`}>
          {awayClinching.icon} {awayClinching.label}
        </span>
      )}
      {homeClinching && (
        <span className={`${homeClinching.color} flex items-center gap-1`}>
          {homeClinching.icon} {homeClinching.label}
        </span>
      )}
    </div>
  </div>
...
export default function LiveUpdater({
  ...
  const [standingsMap, setStandingsMap] = useState<Map<string, Standing>>();
  ...
  const fetchStandings = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Erreur chargement standings");
      const data = await res.json();
      setStandingsMap(createStandingsMap(data)); // createStandingsMap retourne un Map, pas null
    } catch (err) {
      console.error("Erreur fetch standings:", err);
      setStandingsMap(undefined); // ou new Map() en cas d'erreur
    }
  };
  fetchStandings();
}, []);
...
/>
          <div className="grid gap-4 md:gap-6">
            {liveGames.map((game) => (
              <GameCard key={game.id} game={game} standingsMap={standingsMap} />
            ))}
          </div>
        </section>
      )}
